...
def get_df_from_outcar(filename, expression, positions=None, elements=None, **kwargs):
    """
    filename: OUTCAR
    return: DataFrame, colunm X is , colunm y is fcshifts
    """
    atoms = read(filename)
...
class NMRModel():
    ...
    def train_model(self, test_size=0.2, train_size=None, random_state=None, shuffle=True, stratify=None, epochs=EPOCHS, batch_size=BATCH_SIZE, callback_class=TQDMProgressBar(show_epoch_progress=False), filename=None, save_weights=False):
